...
function previous_run_time($line){
	$time=time();
...
$cronSpecification = $argv[1];
$schedule = CronSchedule::fromCronString(trim($cronSpecification));

$nextruntime = next_run_time($cronSpecification);
$cron_next_run_date = date('M j h:i', $nextruntime);

$previousruntime = previous_run_time($cronSpecification);
$cron_previous_run_date = date('M j h:i', $previousruntime);

print $schedule->asNaturalLanguage()."|".$cron_next_run_date."|".$cron_previous_run_date;
